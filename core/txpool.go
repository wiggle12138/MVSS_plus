...
type Tx_pool struct {
	...
	Not_Locking_TX_Pools map[string][]*Transaction
...
func NewTxPool() *Tx_pool {
	pool := Tx_pool{
		...
		Not_Locking_TX_Pools:            make(map[string][]*Transaction),
	...
	for k, v := range pool.Queue {
		...
		if config.Not_Lock_Acc_When_Migrating {
			...
			if account.Not_Lock_Acc[from] && !v.IsRelay && !v.Relay_Lock && (v.TXmig1_Time < v.RequestTime) {
				...
				pool.Not_Locking_TX_Pools[from] = append(pool.Not_Locking_TX_Pools[from], v)
				account.Not_Lock_Acc_Lock.Unlock()
				continue
			} else if account.Not_Lock_Acc[to] && (v.TXmig1_Time < v.Second_RequestTime) {
				if !config.RelayLock {
					if v.LockTime > 0 {
						v.LockTime2 = time.Now().UnixMilli()
					} else {
					...
					pool.Not_Locking_TX_Pools[to] = append(pool.Not_Locking_TX_Pools[to], v)
					account.Not_Lock_Acc_Lock.Unlock()
					continue
				} else {
					encoded := v.Encode()
					...
					pool.Not_Locking_TX_Pools[to] = append(pool.Not_Locking_TX_Pools[to], decoded)
					// account.Lock_Acc_Lock.Unlock()
					// continue
				}
			}
			account.Not_Lock_Acc_Lock.Unlock()
		} else if config.Lock_Acc_When_Migrating {
...
func (pool *Tx_pool) LockTX() {
	...
	for _, tx := range pool.Queue {
		...
		if params.Config.Not_Lock_Acc_When_Migrating {
			account.Not_Lock_Acc_Lock.Lock()
			if !tx.IsRelay && !tx.Relay_Lock && account.Not_Lock_Acc[from] && (tx.TXmig1_Time < tx.RequestTime) {
				if tx.LockTime > 0 {
				...
				tx.SenLock = true
				pool.Not_Locking_TX_Pools[from] = append(pool.Not_Locking_TX_Pools[from], tx)
				account.Not_Lock_Acc_Lock.Unlock()
				continue
			}
			account.Not_Lock_Acc_Lock.Unlock()
			...
			account.Not_Lock_Acc_Lock.Lock()
			if account.Not_Lock_Acc[to] && (account.AccountInOwnShard[from] || tx.IsRelay || tx.Relay_Lock) && (tx.TXmig1_Time < tx.Second_RequestTime) {
				if tx.LockTime > 0 {
					tx.LockTime2 = time.Now().UnixMilli()
				} else {
					tx.LockTime = time.Now().UnixMilli()
				...
				tx.RecLock = true
				pool.Not_Locking_TX_Pools[to] = append(pool.Not_Locking_TX_Pools[to], tx)
				account.Not_Lock_Acc_Lock.Unlock()
				continue
			}
			account.Not_Lock_Acc_Lock.Unlock()
		...
		//半锁
		if !params.Config.Not_Lock_Acc_When_Migrating && !params.Config.Lock_Acc_When_Migrating {
			// 如果不是relay且sender被锁，则放入锁池
			account.Outing_Acc_Before_Announce_Lock.Lock()
...
func TxPoolDeepCopy(dst *[]*Transaction, src []*Transaction) {
	for _, tx := range src {
		*dst = append(*dst, &Transaction{
			...
			TXmig1_Time:          tx.TXmig1_Time,
			TXmig2_Time:          tx.TXmig2_Time,
