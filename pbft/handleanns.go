...
func (p *Pbft) handleAnns(anns []*core.TXann, st *trie.Trie, migTree *trie.Trie) (int64, int64) {
	...
	for _, ann := range anns {
		...
		isdelete[ann.Address] = params.ShardTableInt2Str[ann.ToshardID] //待删除列表
		...
		if params.Config.Not_Lock_Acc_When_Migrating {
			j := 0
			account.Not_Lock_Acc_Lock.Lock()
			for _, tx := range p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[ann.Address] {
				if tx.UnlockTime > 0 {
					tx.UnlockTime2 = time.Now().UnixMilli()
				} else {
					tx.UnlockTime = time.Now().UnixMilli()
				}
				// 如果交易是接收者且不是中继交易或中继锁定交易，则进一步检查发送者是否在待删除列表isdelete中或是否已经被锁定。
				//如果是，则将交易标记为SenLock，并重新放入发送者的锁定交易池中。如果发送者不在待删除列表中且没有被锁定，则将交易标记为成功，并放入交易池的队列中
				if hex.EncodeToString(tx.Recipient) == ann.Address && !tx.IsRelay && !tx.Relay_Lock { //分片内交易
					if _, ok := isdelete[hex.EncodeToString(tx.Sender)]; ok || account.Not_Lock_Acc[hex.EncodeToString(tx.Sender)] { //且sender和receiver账户都是被迁移账户
						tx.SenLock = true
						p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[hex.EncodeToString(tx.Sender)] = append(p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[hex.EncodeToString(tx.Sender)], tx)
						continue
					}
					tx.Success = true //只有receiver账户是被迁移账户，
					p.Node.CurChain.Tx_pool.Queue = append(p.Node.CurChain.Tx_pool.Queue, tx)
					continue
				}
				// 否则，留在被锁交易中
				p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[ann.Address][j] = tx
				j++
			}
			p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[ann.Address] = p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[ann.Address][:j]
			account.Not_Lock_Acc_Lock.Unlock()
		}
...
func (p *Pbft) mpAndSend(isdelete map[string]string, anns []*core.TXann, cAps map[string]map[string]*ChangeAndPending, st, migTree *trie.Trie) {
	...
	m := jointMessage(cUnchangedState, c)
	utils.TcpDial(m, params.ClientAddr)
	fmt.Println("本节点已将不变的账户数量发送到client\n")
}
