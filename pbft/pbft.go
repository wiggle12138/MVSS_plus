...
func NewLog(shardID string) {
	...
	txlog.Write([]string{"sender", "recipient", "txid", "blockHeight", "request_time", "2nd_request_time", "TXmig1_time", "TXmig2_Time", "commit-request", "lock", "unlock", "lock2", "unlock2", "isSuccess", "SenLock", "RecLock", "HalfLock", "SenSupposeHeight", "RecSupposeHeight", "RelayLock"}) //新修改代码
	txlog.Flush()
...
func (p *Pbft) Propose() {
	...
	for {
		...
		// p.propose()
		p.propose1() //pbftType := "Block"
	}
}

// 由 proposer 节点生成一个新的区块，并广播给其他节点进行共识
func (p *Pbft) propose() {
	//创建一个Request对象，并设置其时间戳和消息ID
	r := &Request{}
	//type Request struct {
	//	  Message
	//	  Timestamp int64
	...
	r.Message.Content = encoded_block //将编码后的区块内容设置到请求对象中
...
// 处理commit消息，确保节点能够正确地接收、确认并处理来自其他节点的区块、分片账户变更和账户状态消息
func (p *Pbft) handleCommit(content []byte) {
	...
	if count > malicious_num*2 && !p.isReply[c.Digest] {
		...
		//Block消息：处理区块消息，包括解码区块、更新区块链、处理交易、记录日志以及发送确认消息给客户端
		if pbftType == "Block" && !ok {
			...
			// 2.处理PBFT中的区块消息,包括解码区块、更新区块链、处理交易、记录日志以及发送确认消息给客户端
			if pbftType == "Block" {
				r := p.messagePool[c.Digest] //从消息池中获取消息，解码区块内容并解析为区块对象
				//type Request struct {
				//Message
				//Timestamp int64
				//}
				...
				pbftend := time.Now().UnixMilli() //记录处理开始时间
				// 本地化存储。修改内存、存储至硬盘
				outbalance := p.Node.CurChain.AddBlock(block) //返回要迁出账户的余额
				...
				if p.Node.nodeID == "N0" {
					...
					for _, v := range block.Transactions {
						...
						if params.Config.Not_Lock_Acc_When_Migrating {
							...
							if params.ShardTable[params.Config.ShardID] == toid {
								...
								} else { //否则，将交易加入已上链交易集，并记录交易的相关信息
									commit_txs = append(commit_txs, v)
									v.CommitTime = pbftend
									var s string
									s = fmt.Sprintf("%v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v", hex.EncodeToString(v.Sender), hex.EncodeToString(v.Recipient), v.Id, block.Header.Number, v.RequestTime-InitTime*1000, v.Second_RequestTime-InitTime*1000, v.TXmig1_Time-InitTime*1000, v.TXmig2_Time-InitTime*1000, v.CommitTime-v.RequestTime, v.LockTime-InitTime*1000, v.UnlockTime-InitTime*1000, v.LockTime2-InitTime*1000, v.UnlockTime2-InitTime*1000, v.Success, v.SenLock, v.RecLock, v.HalfLock, v.Sen_Suppose_on_chain, v.Rec_Suppose_on_chain, v.Relay_Lock) //新修改代码
						...
						if v.IsRelay {
							...
							//如果交易接收者不属于当前分片，则增加中继计数，并将交易标记为需要中继
							relayCount++
							v.IsRelay = true
					...
					if params.Config.Not_Lock_Acc_When_Migrating {
						account.Not_Lock_Acc_Lock.Lock()
						for _, v := range block.TXmig1s {
							account.Not_Lock_Acc[v.Address] = true
							p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[v.Address] = make([]*core.Transaction, 0)
						}
						account.Not_Lock_Acc_Lock.Unlock()
					}

					// 若要锁账户，就把账户锁住,相关交易放入Locking_TX_Pools
					...
					// 不停不锁，要把账户半锁，相关交易放入Outing_Before_Announce_TX_Pools
					...
					if params.Config.Not_Lock_Acc_When_Migrating {
						account.Not_Lock_Acc_Lock.Lock()
						for _, locked := range p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools {
							locked_cnt += len(locked)
						}
						account.Not_Lock_Acc_Lock.Unlock()
					} else if params.Config.Lock_Acc_When_Migrating {
						account.Lock_Acc_Lock.Lock()
			...
			} else if pbftType == "EpochChange" { //用于处理分片账户的变更
				...
				p.isReply[c.Digest] = true //标记该消息已被处理
				//如果当前节点是分片主节点，计算新加入的账户数量，并根据情况调用spropose方法或发送确认消息
				if p.Node.nodeID == "N0" {
					flag := false
					//计算进来的个数
				...
				rt, ns := st.Commit(false) //将内存中的状态树提交到磁盘数据库
...
// 构建交易树和状态树，生成交易和状态的证明。将交易分配到对应的relay池中。将relay交易打包成消息，并通过TCP连接发送给目标分片的主节点
func (p *Pbft) TryRelay(relaytxs, alltxs []*core.Transaction, st *trie.Trie, num int) {
	fmt.Printf("\n第%v个块开始准备relay\n\n", num)
	// use a memory trie database to do this, instead of disk database
	triedb := trie.NewDatabase(rawdb.NewMemoryDatabase())
	transactionTree := trie.NewEmpty(triedb)
	for _, tx := range alltxs { //创建一个空的交易树（transactionTree），并遍历所有交易（alltxs），将每个交易的哈希和编码后的交易数据更新到交易树中
	...
	for _, v := range relaytxs { //根据交易的目标分片ID（shardID）将交易分配到对应的relay池中。
...
func (p *Pbft) TryRelay2(relaytxs []*core.TXrelay) {
	config := params.Config
	relaypool := make([][]*core.TXrelay, config.Shard_num)
	for _, v := range relaytxs { //根据交易的目标分片ID将交易分类
...
// 处理从其他分片接收到的交易，并根据接收者所属的分片决定是直接处理还是转发给目标分片
func (p *Pbft) handleRelay(content []byte) {
	...
	relaytx2 := make([]*core.TXrelay, 0) //用于存储需要转发到其他分片的交易
	...
	for _, tx := range relay.Txs {
		...
		account.Not_Lock_Acc_Lock.Lock()
		if !params.Config.Not_Lock_immediately && account.Not_Lock_Acc[hex.EncodeToString(txcs.Recipient)] {
			if txcs.LockTime > 0 {
			...
			p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[hex.EncodeToString(txcs.Recipient)] = append(p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools[hex.EncodeToString(txcs.Recipient)], txcs)
			account.Not_Lock_Acc_Lock.Unlock()
			continue
		}
		account.Not_Lock_Acc_Lock.Unlock()
		...
		account.Lock_Acc_Lock.Unlock()
		if target_shardID == self_shardID { //如果接收者属于当前分片，则将交易添加到txcss中
			txcss = append(txcss, txcs)
			txcs.Second_RequestTime = time.Now().UnixMilli()
		} else { //否则，将交易添加到relaytx2中
			relaytx2 = append(relaytx2, tx)
		}
	}
	p.Node.CurChain.Tx_pool.AddTxs(txcss) //将txcss中的交易添加到本地交易池中
	p.Node.CurChain.Tx_pool.Relaypoollock.Unlock()
	if len(relaytx2) != 0 {
		p.TryRelay2(relaytx2) //调用TryRelay2方法转发relay交易
	}
	// fmt.Printf("\n本节点结束接收relay交易，数量为：%v \n\n", len(relay.Txs))

}

// 验证TXmig1交易的有效性，并将TXmig2发送给目标分片的主节点
func (p *Pbft) TryTXmig1(txmig1s []*core.TXmig1, outbalance map[string]*big.Int, st, migTree *trie.Trie) {
	...
	for _, v := range txmig1s { //根据交易的目标分片ID（ToshardID）将交易分类
		shardID := v.ToshardID
		hex_address, _ := hex.DecodeString(v.Address)
		proofDB1 := &core.ProofDB{}
...
func (p *Pbft) TrySendChangesAndPendings(nss []*core.TXns, caps map[string]*ChangeAndPending, target_shardID string) {
	...
	message := jointMessage(cCaP, bc) //ChangesAndPendings
	go utils.TcpDial(message, target_leader)

}

// 目标分片主节点收到迁移账户余额变化和pending交易并处理
func (p *Pbft) handleChangesAndPendings(content []byte) {
	...
	for _, cap := range caps.List {
		...
		for _, v := range cap.PendingTxs {
			...
			// }
			v.Success = true
			v.TXmig2_Time = time.Now().UnixMilli()
			p.Node.CurChain.Tx_pool.Lock.Lock()
	...
	fmt.Println("本节点已将迁移过来的pending交易的账户数量发送到client\n")
}

// 将当前节点交易池中的交易信息发送给客户端
