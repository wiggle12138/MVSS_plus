...
func (p *Pbft) commit1(content []byte, pbftType string) {
	...
	if pbftType == "Block" {
		...
		if p.Node.nodeID == "N0" {
			if !params.Config.Stop_When_Migrating && !params.Config.Lock_Acc_When_Migrating { //Fine-Tuned
				account.BalanceBeforeOutLock.Lock()
				for k, v := range outbalance {
					account.BalanceBeforeOut[k] = new(big.Int)
					account.BalanceBeforeOut[k].Set(v) //记录要迁出账户的余额
				}
			...
			for _, v := range block.Transactions {
				...
				// txlog.Write(strings.Split(s, " "))
				if v.IsRelay { //如果交易需要中继，则增加中继计数，并将交易加入中继交易集
					relayCount++
				} else if toid != params.ShardTable[params.Config.ShardID] {
					//如果交易接收者不属于当前分片，则同样增加中继计数，并将交易标记为需要中继
					relayCount++
					v.IsRelay = true
			...
			//记录接收账户的时间
			for _, mig := range block.TXmig2s { //TXmig2s
				acc := mig.Address
			...
			//记录TXmig1的时间
			if !params.Config.Stop_When_Migrating { //TXmig1s
			...
			if params.Config.Fail && params.Config.Fail_Time == p.sequenceID && params.Config.ShardID == "S0" {
				if params.Config.Lock_Acc_When_Migrating { //锁
					...
					account.Lock_Acc_Lock.Unlock()
				} else if params.Config.Not_Lock_Acc_When_Migrating { 
					account.Not_Lock_Acc_Lock.Lock()
					account.Not_Lock_Acc["489338d5e8d42e8c923d1f47361d979503d4ad68"] = false
					for _, v := range p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools["489338d5e8d42e8c923d1f47361d979503d4ad68"] {
						v.UnlockTime = pbftend
						v.Success = false
					}
					p.Node.CurChain.Tx_pool.AddTxs(p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools["489338d5e8d42e8c923d1f47361d979503d4ad68"])
					delete(p.Node.CurChain.Tx_pool.Not_Locking_TX_Pools, "489338d5e8d42e8c923d1f47361d979503d4ad68")
					account.Not_Lock_Acc_Lock.Unlock()
				} else { // 不停不锁
					account.Outing_Acc_Before_Announce_Lock.Lock()
		...
		out := make(map[string]int) //需要迁移出去的账户
