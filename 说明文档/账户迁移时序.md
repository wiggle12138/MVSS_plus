...
1. `handleNewMap` 触发迁移，`mvssBuildMigCtx` 建立上下文。  
2. `TXmig1 -> TXmig2 -> Announce -> CaP` 骨架与 MVSS 一致。  
3. 每块提交后 `pbftsingle.commit1 -> mvssTriggerSyncIfNeeded`：  
   - 检测交错并筛选 `FSM=PauseOld` 账户；  
   - `IsMVSSDelta()` 为真时构造 `TXsyncDelta` 并 `TrySendTXsyncDelta`。  
4. 目标片 `handleTXsyncDelta`：  
   - `mvssApplyDeltaSync` 校验并写入 `MigPendingDelta`；  
   - 成功则回 `Ack`；失败则 `mvssAbortDelta`。  
5. 源片收到 `Ack` 后 `mvssOnDeltaAck`：恢复 FSM，清暂停集，推进 `LastCN`。  
6. 目标片出块时 `chain.getUpdatedTreeOfState`：  
   - `TakeMigPendingDelta` 应用 delta（余额/nonce）；  
   - 非法则标记 abort。  
7. `mvssOnAnnounceDone` 在迁移完成后统一清理 `MigCtx`、pending delta、abort 标记。
...
flowchart TD
    ...
    B --> C[commit 后 mvssTriggerSyncIfNeeded]
    C --> D{DetectInterleave 且 IsMVSSDelta?}
    D -- 否 --> E[与 MVSS 主流程一致]
    D -- 是 --> F[构造 TXsyncDelta 并发送]
    F --> G[目标片 handleTXsyncDelta]
    G --> H{mvssApplyDeltaSync 校验}
    H -- 成功 --> I[写入 MigPendingDelta + 回 Ack]
    H -- 失败 --> J[mvssAbortDelta 直接中止]
    I --> K[源片 mvssOnDeltaAck 恢复 FSM]
    K --> L[目标片出块 TakeMigPendingDelta 应用]
    L --> M[Announce 完成后 mvssOnAnnounceDone 清理]
```
